...
from Kermol.kernels.kernels import build_global_kernels, build_local_kernels, grid_search_cv_global, grid_search_cv_local, krr_predict
...
trunc_lambda_grid = [10 ** (-n) for n in range(3, 16)]   
...
def parse_args():
    ...
    p.add_argument('--n_val', type=int, default=300, help='Held out from n_train for per-k lambda tuning.')
...
output_dir = 'truncated_kernel_results'
...
def main():
    ...
    if is_local:
        best = grid_search_cv_local(X_train, y_train, length_grid, lambda_grid, 'gaussian', cv=args.cv_folds, Z=Z_train)
    else:
        best = grid_search_cv_global(X_train, y_train, length_grid, lambda_grid, 'gaussian', cv=args.cv_folds, norm=2)
    print(f"  sigma={best['length']:.4g}  lambda={best['lambda']:.2e}  CV MAE={best['mae']:.4f}")


    if is_local:
        K_train, K_test = build_local_kernels(X_train, X_test, best['length'], 'gaussian',  Z_tr=Z_train, Z_te=Z_test)
    else:
    ...
    rows = [{'k': args.n_train, 'pct': 1.0, 'val_mae': best['mae'], 'test_mae': mae_full, 'test_r2': r2_full}]

    k_values = build_schemes(args.n_train)['LogSweep']
    for k in k_values:
        if k >= args.n_train:
            continue
        lam, val_mae = tune_lambda(eigvecs, eigvals, tr_mask, val_mask, y_train, trunc_lambda_grid, k)
        ...
        rows.append({'k': k, 'pct': round(k / args.n_train, 4), 'val_mae': val_mae, 'test_mae': test_mae, 'test_r2': test_r2})

    df = pd.DataFrame(rows).sort_values('k')
    out_csv = os.path.join(output_dir, f'truncated_kernel_{args.mode}.csv')
