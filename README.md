...
Kernel-ridge regression (KRR) on molecular properties, with a focus on the **spectral behaviour** of the kernel matrix itself: how well does a kernel capture the intinsic structur of the target function, how much of that spectrum is actually needed (truncation), and how robust different molecular representations are to feature removal (ablation).

## Representations and kernels

| Category | Names | Notes |
|---|---|---|
| Global 3D | `coulomb_matrix`, `bob`, `slatm` | Use `build_global_kernels` — Gaussian/Laplacian/linear via pairwise distance |
| Local 3D | `soap`, `acsf`, `fchl19` | Use `build_local_kernels` ([qml2](https://github.com/qml2code/qml2)). Pass nuclear charges (`Z_tr`/`Z_te`) to get the Kronecker-delta ("dn") variant, which only lets same-element atom pairs contribute — the physically correct choice for heteronuclear molecules |
...
| Transformer-based  | `selfies_ted`, `grover_base`, `grover_large`, `chembert`, `chemberta`, `selformer` | **Load** any of them with `load_precomputed_rep` float32 array, used as a global vector representation. |
